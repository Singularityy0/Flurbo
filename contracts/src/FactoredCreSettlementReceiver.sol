...
import {ICreReceiver} from "./CreSettlementReceiver.sol";

/// @notice Version-2 CRE boundary for FactoredPool and FundedFactoredPool (up to 32 events).
/// @dev Locally tested only; deployment and authenticated oracle delivery remain unverified.
/// @dev No authentication bypass, authority rotation, corrections, or direct admin resolution.
contract FactoredCreSettlementReceiver is ICreReceiver, ReentrancyGuard {
    ...
    function bindPool(address target) external {
        ...
        FactoredPool candidate = FactoredPool(target);
    ...
    function onReport(bytes calldata metadata, bytes calldata report) external nonReentrant {
        ...
        if (result.version != 2) revert InvalidReport();
        if (
