...
export default function Pilot({onBusy}:{onBusy(value:boolean):void}) {
  ...
  return <div className="portfolio-view kuru-view pilot-view">
    ...
    {state&&<>
      ...
      <section className="portfolio-wallet pilot-rules"><h2>{state.manifest.publication.draft.title}</h2><p>Trading closes {date(state.manifest.publication.draft.closesAt)}. Pool holds {cash(state.poolCash)} test AUSD against {cash(state.requiredCollateral)} required for payouts.</p><details><summary>Read the rules and panel</summary><p>{state.manifest.publication.draft.exceptionPolicy}</p><p>{state.manifest.publication.draft.disputePolicy}</p><p>Pool {state.manifest.pool}</p><p>Resolver {state.manifest.resolver}</p><p>These reviewers are trusted people. Test bonds and account creation do not provide decentralized security.</p></details></section>
