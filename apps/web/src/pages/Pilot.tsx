...
import { pilotRequest as request, pilotMera, submitPilot, checkPilotPending, isPracticeNamespace, readPilotPending as readPending, pendingKeyFor, validatePilotReview,
...
export default function Pilot({onBusy,namespace='pilot',consumer,onTradeConfirmed,onTradingWalletChange}:{onTradingWalletChange?:(address:string)=>void;onTradeConfirmed?:()=>void;onBusy(value:boolean):void;namespace?:PilotNamespace;consumer?:{event:number;yes:boolean}}) {
  ...
  if(consumer){
    ...
    return <div className="consumer-ticket">
      ...
      {question&&<details><summary>How this market works</summary><p>{question.yesRule}</p><p>{question.noRule}</p><p>{state!.manifest.publication.draft.exceptionPolicy}</p><p>{isPracticeNamespace(namespace)?'These practice outcomes are scripted. ':''}The testnet dispute panel is controlled by Flurbo's operator.</p><a href={question.source.referenceUrl} target="_blank" rel="noreferrer">Read resolution source</a></details>}
    </div>;
  }
  return <div className="portfolio-view kuru-view pilot-view">
    <section className="kuru-intro"><span className="eyebrow">{isPracticeNamespace(namespace)?'Scripted rehearsal / Test assets':'Real events / Test assets'}</span><h2>From a question <em>to an outcome.</em></h2><p>{isPracticeNamespace(namespace)?'A separate practice pool with scripted outcomes. These are not real-world predictions. Your real-event holdings stay in their original pool.':'One shared pool for this cluster. Official-source evidence, an open challenge period and a named testnet reviewer panel.'}</p>{isPracticeNamespace(namespace)&&<p><a href="/rehearsal-rules" target="_blank" rel="noreferrer">Read the fixed rehearsal instructions</a>. A: YES without a challenge. B: propose YES, challenge with NO, then two reviewers vote NO. C: no assertion, then VOID at its deadline. D: YES without a challenge.</p>}<p role="status" className="kuru-notice">{notice}</p><button className="button button-outline" disabled={busy} onClick={()=>void run(()=>refresh())}>Refresh pilot</button></section>
...
function consumerClaim(input:PilotInput,state:PilotState|null){
  ...
  return describeClaim(input.scope,Number(input.mask)).replace(/([A-Z]) (YES|NO)/g,(_match,letter:string,answer:string)=>{
    ...
    return event?`${event.question} ${answer==='YES'?'Yes':'No'}`:letter+' '+answer;
  });
}
