...
import {encodeClaim, type ClaimRule} from '../../shared/claims.mjs';
...
import { pilotRequest as request, submitPilot, checkPilotPending, readPilotPending as readPending, pendingKeyFor, validatePilotReview,
  type Provider, type PilotState, type PilotInput, type PilotReview, type PilotPending, type PilotNamespace, type HoldingWitness } from '../pilot';
...
export default function Pilot({onBusy,namespace='pilot',consumer,challenge,payout,onTradeConfirmed,onTradingWalletChange}:{onTradingWalletChange?:(address:string)=>void;onTradeConfirmed?:()=>void;onBusy(value:boolean):void;namespace?:PilotNamespace;consumer?:{event:number;yes:boolean};challenge?:{event:number};payout?:{scope:number;mask:string;owners:string[];title:string}}) {
  ...
  const [legs,setLegs]=useState<number[]>(restored?.legs.slice().sort((a,b)=>a-b)||[consumer?.event??0]),[quantity,setQuantity]=useState(restored?.quantity||'1'),[side,setSide]=useState<string>(payout?'redeem':restored?.side==='sell'?'sell':'buy');
  const [payoutAmount,setPayoutAmount]=useState('');
  const [rule,setRule]=useState<ClaimRule>(restored?.rule||'AND'),[answers,setAnswers]=useState<Record<number,boolean>>(restored?.answers||(consumer?{[consumer.event]:consumer.yes}:{}));
  ...
  let encodedMask='0';
  try{if(legs.length)encodedMask=encodeClaim({events:state?.manifest.publication.draft.events.length??4,rule:legs.length===1?'AND':rule,legs:legs.map(event=>({event,yes:answers[event]??true}))}).mask;}catch{/* Invalid stored input cannot become a quote or a transaction. */}
  const mask=payout?.mask??encodedMask;
  ...
  useEffect(()=>{
    ...
    try{
      ...
      else saveCheckout(namespace,auth.address,{event:consumer.event,yes:consumer.yes,legs,answers,rule:legs.length===1?'AND':rule,quantity,side:side as CheckoutDraft['side'],walletKind:'browser'});
    }catch{setStorageError(true);setNotice('Your browser could not save this purchase. Enable site storage before continuing.');}
  },[!!consumer,auth.address,legs,answers,rule,quantity,side,choice,completed]);
  useEffect(()=>{
    if(!consumer)return;
    const version=++quoteVersion.current,controller=new AbortController();
    setReview(null);reviewRef.current=null;setQuoting(false);
    if(overTradeLimit||mask==='0')return;
  ...
  async function checkEligibility(){
    const version=generation.current;
    setEligibilityNotice('Checking shares across your linked wallets...');
    try{
  ...
  async function confirm(){
    ...
    if(challenge&&review.requested.action==='dispute'){
      ...
      const reason=challengeUnavailable(fresh,event,owner);
      if(reason)throw Error(reason);
      if(!stake||!(await pilotRequest<{eligible:boolean}>('challenge-eligibility',{owner,event,stake})).eligible)throw Error('Your account no longer has qualifying shares. Check eligibility again.');
    }
  ...
  if(consumer){
    ...
    return <div className="consumer-ticket">
      ...
      {!pending&&!completed&&<><div className="pilot-fields"><label>Your answer<select aria-label="Your answer" disabled={disabled} value={(answers[consumer.event]??true)?'yes':'no'} onChange={e=>setAnswers(old=>({...old,[consumer.event]:e.target.value==='yes'}))}><option value="yes">Yes</option><option value="no">No</option></select></label><label>Shares<input value={quantity} inputMode="decimal" disabled={disabled} onChange={e=>setQuantity(e.target.value)}/></label></div><label>Action<select value={side} disabled={disabled} onChange={e=>setSide(e.target.value)}><option value="buy">Buy</option><option value="sell">Sell</option></select></label>{mask==='0'?<p role="alert">This saved selection is unavailable. <button className="text-link" onClick={()=>{setLegs([consumer.event]);setRule('AND');}}>Reset prediction</button></p>:<CombinationBuilder state={state} base={consumer.event} legs={legs} answers={answers} rule={legs.length===1?'AND':rule} scope={scope} mask={mask} disabled={disabled} namespace={namespace} onLegs={next=>{setLegs(next);if(next.length===1)setRule('AND');}} onAnswer={(event,yes)=>setAnswers(old=>({...old,[event]:yes}))} onRule={setRule}/>}</>}
      ...
      {question&&<details><summary>How this market works</summary><p>{question.yesRule}</p><p>{question.noRule}</p><p>{state!.manifest.publication.draft.exceptionPolicy}</p><p>{state?.manifest.publication.mode==='rehearsal'?'These practice outcomes are scripted. ':''}The testnet dispute panel is controlled by Flurbo's operator.</p><a href={question.source.referenceUrl} target="_blank" rel="noreferrer">{state?.manifest.publication.mode==='ethereum-activity'?'Ethereum API documentation':'Read resolution source'}</a></details>}
    </div>;
  }
