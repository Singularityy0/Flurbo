...
export default function VisitorStatus(){
  ...
  return <section className="home-visitor-status" aria-label="Preview availability">
    ...
    <p>Sign in with a passkey to browse markets and combinations, and view your portfolio. {closed?'Buying is closed for this round. You can view recorded prices where available or choose another collection.':'View current prices and confirm trades with MetaMask while a market is open.'}</p>
    <p className="home-muted">No opening date for a new practice round is announced here.</p>
    <div className="home-status-actions"><Link href="/markets" className="text-link">Browse markets ↗</Link><Link href="/portfolio" className="text-link">Portfolio</Link><button type="button" className="text-link" disabled={loading} onClick={()=>setAttempt(value=>value+1)}>Refresh status</button></div>
  </section>;
}
