...
export default function MarketDetail({namespace,event}:{namespace:PilotNamespace;event:number}){
  ...
  return <main id="main" tabIndex={-1} className="markets-page market-detail">
    ...
    {question&&view&&state&&<>
      <header className="detail-heading"><div><span className="market-badge">{namespace==='pilot'?'Monad testnet':'Practice · Monad testnet'}</span><h1>{question.question}</h1></div><span className="detail-status">{error?'Status unavailable':view.title}</span></header>
