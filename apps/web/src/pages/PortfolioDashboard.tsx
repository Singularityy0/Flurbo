...
export default function PortfolioDashboard({account,wallets,groups}:{account:string;wallets:string[];groups:MarketGroup[]}){
  ...
  const [tab,setTab]=useState<'Holdings'|'Activity'|'Collected'|'Performance'>('Holdings'),[filter,setFilter]=useState('All'),[page,setPage]=useState(0),[payoutBusy,setPayoutBusy]=useState(false);
  ...
  const allPayouts=groups.every(g=>data[g.namespace]?.payouts?.complete&&!data[g.namespace]?.payoutError);
  const payoutReadFailed=Object.values(data).some(d=>d.payoutError);
  const trades=useMemo(()=>groups.flatMap(g=>accountActivity(g.namespace,g.pool,[...(data[g.namespace]?.history?.logs||[]),...(data[g.namespace]?.payouts?.logs||[])],identity.split(','))).sort((a,b)=>a.block-b.block||a.index-b.index),[data,groups,identity]);
  ...
  const collected=trades.filter(t=>t.action==='Payout');
  const collectedTotal=collected.reduce((sum,t)=>sum+t.cash,0n);
  const activity=tab==='Collected'?collected:trades;
  const shown=[...activity].reverse().slice(page*20,page*20+20);
  return <div className="unified-portfolio portfolio-view">
    ...
    <div className="portfolio-summary-grid">
      ...
      <article><span>Payouts collected</span><strong>{collected.length?amount(String(collectedTotal)):allPayouts?'0':payoutReadFailed?'Unavailable':'Checking...'}</strong><small>{allPayouts?'Test AUSD received':collected.length?'Test AUSD found so far':payoutReadFailed?'Refresh to check past payouts':'Reading payout transactions'}</small>{collected.length>0&&<button className="portfolio-collected-link" onClick={()=>{setTab('Collected');setPage(0);}}>View collected payouts</button>}</article>
    </div>
    <div className="portfolio-tabs" aria-label="Portfolio views">{(['Holdings','Activity','Collected','Performance'] as const).map(value=><button key={value} aria-pressed={tab===value} onClick={()=>{setTab(value);setPage(0);}}>{value}{value==='Holdings'?' · '+rows.length:''}</button>)}</div>
    {tab==='Performance'?<><section className="portfolio-performance" aria-label="Realized PnL">
      ...
      {pnlReady&&performance.points.length>0?<PnlChart points={performance.points}/>:<div className="portfolio-chart-empty"><p>{!pnlReady?'Your chart will appear when trading history is verified.':'No realized profit or loss yet.'}</p><small>{pnlReady?'Sales and collected payouts will appear here.':'Your holdings can be used while earlier activity loads.'}</small></div>}
    ...
    </section></>:tab==='Holdings'?<section aria-label="Your shares"><div className="portfolio-section-heading"><h2>Your shares</h2><label className="portfolio-filter">Show <select value={filter} onChange={e=>setFilter(e.target.value)}>{['All','Unsettled','Ready to collect','Settled'].map(f=><option key={f}>{f}</option>)}</select></label></div>
    ...
    </section>:<section aria-label={tab==='Collected'?'Your collected payouts':'Your trading activity'}><div className="portfolio-section-heading"><h2>{tab==='Collected'?'Collected payouts':'Your trades'}</h2><span>{tab==='Collected'?(allPayouts?'Verified payout transactions':payoutReadFailed?'Payout history unavailable':'Checking payout history'):(allHistory?'Indexed activity':'History still loading')}</span></div><div className="portfolio-table portfolio-activity"><table><thead><tr><th>Prediction / action</th><th>Shares</th><th>Test AUSD</th><th>Transaction</th></tr></thead><tbody>{shown.map(trade=><tr key={`${trade.pool}:${trade.hash}:${trade.index}`}><td><span className="portfolio-tag">{trade.action==='Payout'?'Collected':trade.action}</span><strong>{label(trade.namespace,trade.scope)}</strong><small>{describeClaimAnswers(trade.scope,Number(trade.mask))}</small></td><td>{amount(String(trade.quantity))}</td><td>{trade.action==='Buy'?'-':'+'}{amount(String(trade.cash))}</td><td><a href={`https://testnet.monadexplorer.com/tx/${trade.hash}`} target="_blank" rel="noreferrer">View transaction ↗</a><small>Block {trade.block.toLocaleString()}</small></td></tr>)}</tbody></table></div>{!shown.length&&<p className="portfolio-footnote">{tab==='Collected'?(allPayouts?'No payouts collected yet.':payoutReadFailed?'Past payouts could not be checked. Refresh to retry. Your funds and current holdings are unaffected.':'Checking previous payout transactions. Zero remaining shares does not mean no payout was collected.'):(allHistory?'No confirmed trades found for your linked wallets.':'Confirmed trades will appear as history loads.')}</p>}<div className="portfolio-pagination"><button className="button button-outline" disabled={!page} onClick={()=>setPage(p=>p-1)}>Previous</button><span>Page {page+1}</span><button className="button button-outline" disabled={(page+1)*20>=activity.length} onClick={()=>setPage(p=>p+1)}>Next</button></div></section>}
