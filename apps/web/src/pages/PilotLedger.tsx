...
export default function PilotLedger({account,history,namespace='pilot'}:{account:string;history:boolean;namespace?:PilotNamespace}) {
  ...
  async function refresh(next=owner,p=page){
    if(working.current)return;working.current=true;const generation=++version.current;setBusy(true);
    try{
      ...
      setNotice(i.complete?'Activity indexed through the displayed checkpoint. Holdings are read directly from the pool.':'Loading earlier activity. Discovered positions and history are incomplete until indexing catches up.');
    }catch(e){if(generation===version.current){setCatchingUp(false);setNotice(e instanceof Error?e.message:'Pilot data unavailable');}}
    finally{working.current=false;if(generation===version.current)setBusy(false);}
  }
  ...
  return <div className="portfolio-view"><section className="portfolio-wallet"><label>Wallet to view<input value={wallet} disabled={busy} onChange={e=>setWallet(e.target.value)}/></label><button className="button button-dark" disabled={busy} onClick={()=>{setCatchingUp(true);void refresh(wallet,0);}}>View wallet</button><button className="button button-outline" disabled={busy} onClick={()=>{setWallet(account);setCatchingUp(true);void refresh(account,0);}}>Use Mera wallet</button><p>Read-only view. Your Mera and MetaMask addresses hold separate positions.</p></section><p role="status">{notice}</p>{index&&!index.complete&&<section className="portfolio-wallet"><p>{Math.max(0,index.target-index.through).toLocaleString()} blocks remaining. {catchingUp?'Continues automatically while this page is visible.':'Automatic loading is paused.'} No transactions are sent.</p><button className="button button-outline" onClick={()=>setCatchingUp(value=>!value)}>{catchingUp?'Pause loading':'Continue loading'}</button></section>}{state&&<><h2>{state.manifest.publication.draft.title}</h2>{state.manifest.publication.reviewerControl==='single-operator'&&<p>Operator-run testnet alpha. All reviewer wallets are controlled by one operator; dispute resolution is not independent.</p>}<p>{amount(state.wallet?.cash)} test AUSD in {owner}.</p><details><summary>Event key</summary>{state.manifest.publication.draft.events.map((e,i)=><p key={e.id}>{String.fromCharCode(65+i)}: {e.question}</p>)}</details><p>Indexed through block {index?.through}, target {index?.target}. <button disabled={busy} onClick={()=>{setCatchingUp(true);void refresh();}}>Refresh</button></p>{history?<><p>Your wallet activity and the shared public resolution record for this cluster.</p><div className="portfolio-table"><table><thead><tr><th>Action</th><th>Block</th><th>Transaction</th></tr></thead><tbody>{activity.slice(page*30,page*30+30).map(e=><tr key={e.hash+e.index}><td>{e.name}{['Asserted','Disputed','Voted'].includes(e.name)&&<><br/>Outcome: {['Unset','NO','YES','VOID'][Number(e.args.outcome)]}<br/>{evidenceURI(e.args.evidenceURI||e.args.rationaleURI)&&<a href={String(e.args.evidenceURI||e.args.rationaleURI)} target="_blank" rel="noreferrer">Read evidence or rationale</a>}</>}</td><td>{e.block}</td><td><a href={`https://testnet.monadscan.com/tx/${e.hash}`} target="_blank" rel="noreferrer">{e.hash.slice(0,12)}...</a></td></tr>)}</tbody></table></div>{!activity.length&&<p>{index?.complete?'No indexed activity for this address.':'No activity found in the scanned blocks yet.'}</p>}</>:<><div className="portfolio-table"><table><thead><tr><th>Claim</th><th>Shares</th><th>Redeemable AUSD</th></tr></thead><tbody>{rows.filter(r=>r.quantity!=='0').map(r=><tr key={`${r.scope}:${r.mask}`}><td>{describeClaim(r.scope,Number(r.mask))}</td><td>{amount(r.quantity)}</td><td>{r.payoutAtoms===null?'Pending':amount(r.payoutAtoms)}</td></tr>)}</tbody></table></div>{!rows.some(r=>r.quantity!=='0')&&<p>{index?.complete?'No shares among this page of discovered claims.':'No shares among the claims discovered so far. Indexing is incomplete.'}</p>}<Link href={namespace==='rehearsal'?'/rehearsal':'/events'}>Trade or redeem in {namespace==='rehearsal'?'the rehearsal':'Real events'}</Link></>}<div className="pilot-actions"><button disabled={busy||page===0} onClick={()=>void refresh(owner,page-1)}>Previous</button><button disabled={busy||(page+1)*30>=(history?activity.length:claimCount)} onClick={()=>void refresh(owner,page+1)}>Next</button></div></>}</div>;
}
