...
export async function runMockReadiness({args=process.argv.slice(2),env=process.env,request=fetch,output=console.log}={}){
  if(args.length>1||args.length===1&&!['--rehearsal','--october-demo'].includes(args[0]))throw Error('Use --rehearsal or --october-demo, or no flag for the real pilot');
  const name=args[0]==='--october-demo'?'october-demo':args[0]==='--rehearsal'?'rehearsal':'pilot';
  ...
  const namespace=name==='october-demo'?`practice-${manifest.pool.slice(2)}`:name;
  ...
  const report=await checkMockReadiness({manifest,namespace,featured:name==='october-demo',request,rpc,model:input=>runPairModel(input,{file})});
