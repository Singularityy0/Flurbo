...
export type PilotManifest = { status: string; pool: `0x${string}`; resolver: `0x${string}`; rulesHash: string; draftHash: string;
  ...
  publication: {creator: string; mode?: 'official-releases'|'rehearsal'; reviewerControl?: 'independent-panel'|'single-operator'; bondAtoms: string; assertionPeriod: number; challengePeriod: number; votingPeriod: number;
