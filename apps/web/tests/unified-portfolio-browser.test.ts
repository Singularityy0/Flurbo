 ...
 try{
  ...
  await page.route('**/*',async(route:any)=>{
   ...
   if(p.endsWith('/positions')){
    const q=route.request().postDataJSON();assert.ok(q.claims.length<=8);active++;max=Math.max(max,active);await new Promise(r=>setTimeout(r,15));active--;
  ...
  complete=true;await page.getByRole('button',{name:'Load more activity'}).click();await page.getByRole('img',{name:/Realized PnL after 2.*\+4 test AUSD/}).waitFor();assert.equal(accounts,2);
  ...
  for(const width of [1440,390]){await page.setViewportSize({width,height:1000});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:new URL(`../../../target/unified-portfolio/portfolio-${width}.png`,import.meta.url).pathname.replace(/^\/([A-Z]:)/,'$1'),fullPage:true});}
  ...
  fail=false;await page.getByRole('button',{name:'Refresh',exact:true}).click();await page.getByRole('img',{name:/Realized PnL after 2/}).waitFor();assert.deepEqual(errors,[]);
