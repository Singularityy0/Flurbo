...
export function rehearsalEvent(bit:number,startsAt:number,endsAt:number):EventDraft['events'][number] {
  if(![0,1,2].includes(bit))throw new Error('Unknown rehearsal event');
  return {id:`rehearsal-${bit}`,question:`Rehearsal ${String.fromCharCode(65+bit)}: does the scripted fixture report YES?`,
    ...
    source:{publisher:'Flurbo test operator',referenceUrl:'https://flurbo.singu.online/rehearsal-rules',recordId:`flurbo-rehearsal.v1:${bit}`,
      selectionRule:bit===0?'Scripted fixture A is YES. Exercise an unchallenged assertion.':bit===1?'Scripted fixture B is NO. Intentionally assert YES, challenge with NO, then have two reviewers vote NO.':'Scripted fixture C has no result. Submit no assertion; finalize VOID after the assertion deadline.',
...
export function validateRehearsalDraft(draft:EventDraft) {
  if(draft.title!==REHEARSAL_TITLE||!draft.clusterId.startsWith('public-rehearsal-')||draft.events.length!==3)throw new Error('Explicit three-event rehearsal required');
