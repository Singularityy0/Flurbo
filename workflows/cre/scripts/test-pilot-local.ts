...
const policy={reviewers:reviewers.slice(0,3).map((address,i)=>({name:`Fixture person ${i+1}`,address})),bondAtoms:'1000000',assertionPeriod:3600,challengePeriod:3600,votingPeriod:3600};
const input={schema:'flurbo.pilot-publication.v1',creator,independentReviewersConfirmed:true,rulesReviewed:true,...policy,
  draft:{schema:'flurbo.event-draft.v1',status:'draft',chainId:10143,clusterId:'local-test-only',title:'Isolated end-to-end fixtures',closesAt:now+7200,
    events:[releaseEvent({repository:'ethereum/go-ethereum',tag:'v99.0.0'},'geth',now+7300,now+8000),releaseEvent({repository:'paradigmxyz/reth',tag:'v99.0.0'},'reth',now+7300,now+8000)],
...
const pool=await deploy('PilotPool',[cash,2,10_000_000n,BigInt(now+7200),[0,1],resolver,rulesHash]);
...
for(let i=0;i<2;i++) await call(creator,pool,artifacts.PilotPool.abi,'createBaseToken',[i,true]);
...
await action(alice,'buy',{scope:3,mask:'8',quantity:'2000000',slippageBps:50});
let position=await service.position(alice,3,'8');
if(position.quantity!=='2000000')throw new Error('Buy missing from holdings');
await action(alice,'sell',{scope:3,mask:'8',quantity:'1000000',slippageBps:50});
...
const e=await evidence.put({eventId:'geth',outcome:2,statement:'Explicit synthetic test evidence, not a real result.',sourceURL:'https://github.com/ethereum/go-ethereum/releases',attachment:'fixture'},alice,manifest.draftHash);
...
await action(bob,'dispute',{event:0,outcome:1,evidenceHash:e.hash,evidenceURI:e.uri});
for(const reviewer of reviewers.slice(0,2))await action(reviewer,'vote',{event:0,outcome:2,evidenceHash:e.hash,evidenceURI:e.uri});
// Event B has no assertion and becomes void after its deadline.
clock+=3601;await rpc('evm_setNextBlockTimestamp',[clock]);await rpc('evm_mine');
await action(bob,'finalize',{event:1});await action(bob,'deliver');
position=await service.position(alice,3,'8');
if(position.payoutAtoms!=='500000')throw new Error('Partial void payout mismatch');
await action(alice,'redeem',{scope:3,mask:'8',quantity:'1000000'});
