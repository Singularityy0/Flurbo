...
const now=Math.floor(Date.now()/1000),closesAt=now+7200;
...
const publication={schema:'flurbo.pilot-publication.v1',mode:'rehearsal',...policy,
  draft:{schema:'flurbo.event-draft.v1',status:'draft',chainId:10143,clusterId:`public-rehearsal-${now}`,title:REHEARSAL_TITLE,closesAt,
    events:[0,1,2].map(bit=>rehearsalEvent(bit,closesAt+60,closesAt+120)),disputeModel:'reviewer-panel',disputePolicy:disputePolicy(policy),exceptionPolicy:VOID_POLICY}};
...
console.log(JSON.stringify({status:prepared.status,mode:'rehearsal',configHash:prepared.configHash,subsidyTestAusd:'20.794416',
  tradeCloses:new Date(closesAt*1000).toISOString(),assertionsOpen:new Date((closesAt+120)*1000).toISOString(),
  assertionDeadline:new Date((closesAt+3720)*1000).toISOString(),notice:'Deploy within one hour of preparing. No deployment has occurred. Do not overwrite the real pilot manifest.'},null,2));
